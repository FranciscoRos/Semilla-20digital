...
export default function ForoTemas() {
 ...
 const { data: temas, isLoading } = useQuery({
  ...
  const handleCreateTopic = async (e: React.FormEvent) => {
    ...
    // Simulación de API
    console.log("Creando tema:", { title: newTopicTitle, content: newTopicContent, subCat: idSubCategoria });
    ...
    <div className="min-h-screen bg-[#F8FAFC] p-6 md:p-10">
      <div className="max-w-5xl mx-auto space-y-8 animate-pulse">
         ...
         <div className="space-y-4">
           {[1, 2, 3].map(i => (
             <div key={i} className="bg-white h-48 rounded-3xl border border-slate-100 p-8" />
           ))}
         </div>
      </div>
    </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 md:py-10">
        ...
        <div className="mb-10">
            ...
            >
                ...
                <span className="group-hover:translate-x-1 transition-transform">Volver a Categorías</span>
            </button>
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 bg-white p-6 md:p-8 rounded-[2rem] shadow-xl shadow-slate-200/40 border border-slate-100 relative overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <p className="text-slate-500 mt-2 text-lg max-w-xl">
                        Explora ideas, resuelve dudas y conecta con otros miembros de la comunidad.
                    </p>
                </div>

                <div className="flexflex-col gap-3 relative z-10">
                  <button 
                  ...
                  required
                />
                    <button className="hidden md:flex items-center justify-center w-12 h-12 rounded-xl bg-slate-50 text-slate-500 border border-slate-200 hover:bg-white hover:text-green-600 hover:border-green-200 hover:shadow-md transition-all">
        ...
        {/* ================= BARRA DE FILTROS (Opcional visualmente) ================= */}
        <div className="flex items-center gap-2 mb-6 px-2 overflow-x-auto pb-2 scrollbar-hide">
            {['Todos', 'Populares', 'Sin Respuesta', 'Mis Temas'].map((filter, idx) => (
                <button 
                    key={filter}
        ...
        <div className="space-y-5">
          {temas?.length === 0 && (
              <div className="flex flex-col items-center justify-center p-16 text-center bg-white rounded-[2.5rem] border border-dashed border-slate-300">
                  ...
                  <h3 className="text-lg font-bold text-slate-700">Aún no hay temas aquí</h3>
                  <p className="text-slate-500 mb-6 max-w-xs mx-auto">Sé la primera persona en iniciar una conversación interesante en esta categoría.</p>
                  <button 
                    onClick={() => setIsModalOpen(true)}
              ...
              <div className="flex flex-col md:flex-row gap-6">
                  ...
                  <div className="flex-1">
                      <div className="flex flex-wrap gap-2 mb-3">
                          {tema.Tags?.map(tag => (
                              <span key={tag} className="inline-flex items-center gap-1 bg-slate-50 text-slate-500 text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-wide border border-slate-100 group-hover:border-green-100 group-hover:bg-green-50 group-hover:text-green-700 transition-colors">
                                  {tag}
                              </span>
                          ))}
                          {tema.Verificado && (
                      ...
                      <p className="text-slate-500 text-sm mb-4 line-clamp-2 leading-relaxed">
                          Haga clic para ver la discusión completa y participar en el hilo...
                      </p>
                      ...
                      <div className="flex items-center gap-3">
                          <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs font-semibold text-slate-700">{tema.Autor.Nombre}</span>
                          </div>
                          <span className="text-slate-300 text-xs">•</span>
                          <span className="text-xs text-slate-400">{tema.Autor.Ubicacion || 'Comunidad'}</span>
                      </div>
                  </div>
                  ...
                  <div className="flex md:flex-col items-center md:items-end justify-between md:justify-center gap-4 border-t md:border-t-0 md:border-l border-slate-50 pt-4 md:pt-0 md:pl-6 min-w-[120px]">
                      <div className="text-center">
                          <div className="flex items-center gap-1.5 text-slate-400 group-hover:text-green-600 transition-colors mb-1">
                              ...
                              <span className="text-lg font-bold text-slate-700 group-hover:text-green-700">{tema.Comentarios}</span>
                          </div>
                          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Respuestas</span>
                      </div>
                      ...
                      <div className="text-center flex md:block items-center gap-2">
                           <div className="flex items-center gap-1.5 text-slate-400">
                              ...
                              <span className="text-sm font-semibold">{tema.Vistas}</span>
                           </div>
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-xl relative z-10 overflow-hidden animate-in fade-in zoom-in-95 duration-300 border border-slate-100">
            ...
            <div className="flex justify-between items-center p-6 border-b border-slate-50 bg-slate-50/50">
              <div>
                  <h3 className="text-xl font-extrabold text-slate-800">Crear Nuevo Tema</h3>
                  <p className="text-sm text-slate-500">Comparte tus dudas o conocimientos</p>
              </div>
              <button 
